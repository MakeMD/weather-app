...
- ✅ **fix(android):** іконка `↻` refresh не була видима в dark theme. Стиль `icon` не мав `color`, а `↻` — текст-символ (на відміну від emoji ⚙️) і потребує явний колір. Розділила на `icon` (для emoji) і `iconRefresh` (з `color: colors.text`). _components/CityHeader.js_ _(2026-05-10)_
